...
/* Last FTMS speed for grade limiter (in meters per hour) */
uint16_t last_ftms_speed_mh = 0;

/* Last applied grade for thermal learning */
int16_t last_applied_grade = 0;
		    ...
		    const void *data, uint16_t length)
{
	...
	if (svc_type == 0) {
		...
		json_out("{\"type\":\"hr\",\"ts\":%u,\"bpm\":%u}\n", k_uptime_get_32(), heart_rate);
	} else if (svc_type == 1) {
		/* CP service */
		...
		if (length >= 4) {
			...
			json_out("{\"type\":\"cp\",\"ts\":%u,\"power\":%d,\"flags\":%u", last_cp_data_time, power, flags);
		...
		/* Update active time for decay (power > 50W indicates active use) */
		grade_limiter_update_active_time(cached_cp_data.power > 50);
		...
		if (length >= 2) {
			...
			json_out("{\"type\":\"ftms\",\"ts\":%u,\"flags\":%u", now, flags);
			
			/* Bit 8 (0x0100) indicates trainer is releasing resistance (thermal management) */
			if (flags & 0x0100) {
				json_out(",\"releasing\":true");
				releasing = true;
			}
			...
			if (length >= offset + 2) {
				...
				last_ftms_speed_mh = speed;  /* Store for grade limiter */
			...
			/* Learn from thermal release event */
			if (releasing && last_applied_grade > 0) {
				grade_limiter_learn(last_ftms_speed_mh, last_applied_grade);
			}
			
			/* Update active time for decay (trainer connected = active) */
			grade_limiter_update_active_time(true);
		}
